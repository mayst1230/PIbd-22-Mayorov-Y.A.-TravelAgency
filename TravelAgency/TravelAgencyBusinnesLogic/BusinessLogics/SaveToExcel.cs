...
{
    internal static class SaveToExcel
    {
...
SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
               ...
               spreadsheetDocument.WorkbookPart.Workbook.AppendChild<Sheets>(new Sheets());
                ...
                uint rowIndex = 2;
                foreach (ViewModels.ReportTravelConditionViewModel tc in info.TravelConditions)
                {
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        Worksheet = worksheetPart.Worksheet,
        ...
        public static void CreateDocTravelAgency(ExcelInfoTravelAgency info)
        {
            using (SpreadsheetDocument spreadsheetDocument =
                SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
                ...
                foreach (var travelAgencyCondition in info.TravelAgencyConditions)
                {
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        Worksheet = worksheetPart.Worksheet,
                        ...
                        ColumnName = "A",
                        RowIndex = rowIndex,
                        Text = travelAgencyCondition.TravelAgencyName,
                        StyleIndex = 0U
                    });

                    rowIndex++;

                    foreach (var condition in travelAgencyCondition.Conditions)
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            Worksheet = worksheetPart.Worksheet,
                    ...
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        ...
                        ColumnName = "C",
                        RowIndex = rowIndex,
                        Text = travelAgencyCondition.TotalCount.ToString(),
                        StyleIndex = 0U
                    });
