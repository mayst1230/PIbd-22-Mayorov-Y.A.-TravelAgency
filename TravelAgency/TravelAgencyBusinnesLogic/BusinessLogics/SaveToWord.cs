...
{
    internal static class SaveToWord
    {
        /// <summary>
        /// Создание документа
        /// </summary>
           ...
           WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
            {
                ...
                foreach (var travel in info.Travels)
                {
                    docBody.AppendChild(CreateParagraph(new WordParagraph
                    {
                        Texts = new List<(string, WordTextProperties)> { (travel.TravelName + ": ", new WordTextProperties { Size = "24", Bold = true, }), (travel.Price.ToString(), new WordTextProperties { Size = "24", Bold = false, }) },
                        TextProperties = new WordTextProperties
                        {
                            Size = "24",
       ...
       paragraphProperties)
        ...
        public static void CreateDocTravelAgency(WordInfoTravelAgency info)
        {
                ...
                .Create(info.FileName, WordprocessingDocumentType.Document))
            {
                ...
                foreach (var travelAgency in info.Agencies)
                {
                    ...
                    cellName.Append(new TableCellProperties(
                        new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "2400" }));
                    cellName.Append(new Paragraph(new Run(new Text(travelAgency.AgencyName))));

                    TableCell cellPerson = new TableCell();
                    cellPerson.Append(new TableCellProperties(
                        new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "2400" }));
                    cellPerson.Append(new Paragraph(new Run(new Text(travelAgency.FullNameResponsible))));

                    TableCell cellDateCreate = new TableCell();
                    cellDateCreate.Append(new TableCellProperties(
                        new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "2400" }));
                    cellDateCreate.Append(new Paragraph(new Run(new Text(travelAgency.CreationDate.ToString()))));
